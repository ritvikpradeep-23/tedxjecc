    ...
    <div style="max-width:380px;margin:0 auto;">
      ...
      <div style="background:#1a1a1a;border:2px solid #e62b1e;border-radius:12px;overflow:hidden;">
        ...
        <div style="padding:18px 20px;text-align:center;background:#0d0d0d;">
          <img src="${SITE_URL}/api/tickets/${ticket.uuid}/qr" width="140" height="140" alt="Ticket QR code" style="display:block;margin:0 auto;border:4px solid #ffffff;border-radius:8px;" />
...
export async function sendTicketEmail(order) {
  ...
  try {
    ...
    // Only `html` is passed below (no `text` field) — Resend sends this as
    // a proper HTML email, not plain text. Logged here so the exact
    // outgoing markup (and the QR <img> src) is visible in Vercel's
    // function logs for debugging.
    console.log(`Sending ticket email to ${order.email} for ticket ${ticket.uuid}:\n${html}`);
